...
|---|---|---|---|---|---|
| R0 | Negative flow | Physical failure | Physical impossibility | Daily | `src/hydrokg_rules.py::NegativeFlowRule` |
| R1 | Extreme ratio | Predictive error | Magnitude failure | Daily | `src/hydrokg_rules.py::ExtremeRatioRule` |
| R2 | Zero-flow collapse | Predictive error | Physical impossibility | Daily | `src/hydrokg_rules.py::ZeroFlowCollapseRule` |
| R3 | High relative error | Predictive error | Magnitude failure | Daily | `src/hydrokg_rules.py::HighRelativeErrorRule` |
| R4 | Peak-timing error | Predictive error | Timing failure | Event/window | `src/hydrokg_rules.py::PeakTimingRule` |
| R5 | Annual mass balance | Physical failure | Budget-scale failure | Annual | `src/hydrokg_rules.py::MassBalanceRule` |
| R6 | Budyko consistency | Physical failure | Budget-scale failure | Annual | `src/hydrokg_rules.py::BudykoConsistencyRule` |
...
**Added safeguard, not in the manuscript, confirmed necessary on real data:** also
requires `Q_obs` to exceed 5% of the basin's own long-term mean flow (`min_flow_frac`).
Without this floor, R1 (and especially R3) are dominated by low-flow/baseflow days where
a small *absolute* error produces a huge *ratio* purely because `Q_obs` is tiny — verified
directly with a controlled test (a 5% error at high flow triggers nothing; the same
absolute-scale error at low flow triggered every single day). On a real 647-basin run,
this made R3 alone account for 82.84% of all violations across all seven rules, and
`dominant_class` for 95.5% of basins — measuring which basins have more low-flow days,
not which basins violate different physical laws. Report and justify this threshold
explicitly in the manuscript, the same as R2/R3's other added thresholds below.
...
**Added safeguard, confirmed necessary on real data:** same `min_flow_frac` low-flow floor
as R1 (see R1's section above for the full mechanism and the controlled test that
confirmed it). On a real 647-basin run, R3 without this floor accounted for 82.84% of
all violations across all seven rules — driven by low-flow relative-error blowup, not
genuinely severe predictions.
...
`src/hydrokg_rules.py` defines `DAILY_RULES`, `EVENT_RULES`, `ANNUAL_RULES`. Only
`DAILY_RULES` (R0-R3) are evaluated in real time, inside
`EnhancedTrainingPipeline.fine_tune()`'s training loop (`src/hydrokg_enhancement.py`) --
