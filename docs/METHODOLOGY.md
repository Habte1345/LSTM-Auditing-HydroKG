...
1. `src/hydrokg_enhancement.py (ViolationCurriculumSampler)::ViolationCurriculumSampler` — graph query →
...
2. `src/hydrokg_enhancement.py (GraphAnalogyCorrector)::GraphAnalogyCorrector` — graph
   traversal to structurally similar, low-violation basins → post-hoc correction of the
   model's raw output.
3. `src/hydrokg_enhancement.py (build_embedding_matrix)::build_embedding_matrix` — graph query →
   auxiliary static input feature for a fine-tuning pass.

`src/hydrokg_enhancement.py (EnhancedTrainingPipeline)::EnhancedTrainingPipeline` wires all three
...
- `hydrokg_graph.Neo4jGraphStore` against a live server
